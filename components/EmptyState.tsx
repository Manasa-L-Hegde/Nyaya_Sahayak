...
import { Home, ShoppingBag, ShieldCheck, Briefcase, FileSearch, ShieldAlert } from "lucide-react";
...
export const EmptyState: React.FC<EmptyStateProps> = ({ onSelectPrompt, language }) => {
  const examplePrompts = [
    ...
    },
    {
      ...
      color: "border-rose-500/20 hover:border-rose-500/50 bg-rose-500/5 hover:bg-rose-500/10",
    },
    {
      domain: "Labour & Wages",
      ...
      color: "border-amber-500/20 hover:border-amber-500/50 bg-amber-500/5 hover:bg-amber-500/10",
    },
    {
      ...
      color: "border-indigo-500/20 hover:border-indigo-500/50 bg-indigo-500/5 hover:bg-indigo-500/10",
    },
  ];

  return (
    <div className="max-w-3xl mx-auto my-auto py-6 sm:py-8 text-center px-1">
      <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-indigo-500/10 text-indigo-700 dark:text-indigo-400 text-xs font-semibold mb-3 sm:mb-4 border border-indigo-500/20 hover:bg-indigo-500/15 transition-colors cursor-default">
        <span>⚖️ Context-Aware Indian Legal Intelligence</span>
      </div>

      <h2 className="text-xl sm:text-2xl md:text-3xl font-bold tracking-tight text-foreground mb-2 sm:mb-3">
        {language === "hi" ? "आपकी कानूनी समस्या क्या है?" : "How can Nyaya Sahayak assist you today?"}
      </h2>

      <p className="text-xs sm:text-sm text-muted-foreground max-w-xl mx-auto mb-6 sm:mb-8 px-2">
        {language === "hi"
        ...
        {examplePrompts.map((item, idx) => {
          ...
          return (
            <button
              ...
              className={`p-3.5 sm:p-4 rounded-xl border transition-all duration-200 text-left group hover:-translate-y-0.5 hover:shadow-md active:scale-[0.98] cursor-pointer focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 ${item.color}`}
            >
              <div className="flex items-center gap-2 mb-1.5 sm:mb-2">
                <Icon className="w-4 h-4 text-muted-foreground group-hover:text-indigo-600 dark:group-hover:text-indigo-400 group-hover:scale-110 transition-all duration-200" />
